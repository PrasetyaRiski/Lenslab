...
export default async function EditMentorQuizPage({ params }: { params: Promise<{ id: string }> }) {
  const user = await requireRole([Role.MENTOR, Role.ADMIN]);
  const { id } = await params;
  const [quiz, materials] = await Promise.all([
    prisma.quiz.findFirst({ where: { id, ...(user.role === "MENTOR" ? { createdById: user.id } : {}) }, include: { questions: true } }),
    prisma.material.findMany({ where: user.role === "MENTOR" ? { createdById: user.id } : {}, orderBy: { title: "asc" } })
  ]);
  if (!quiz) notFound();
