...
export default async function AdminQuestionsPage() {
  ...
  const quizzes = await prisma.quiz.findMany({ include: { questions: true }, orderBy: { createdAt: "desc" } });
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {quizzes.map((quiz) => (
            <Card key={quiz.id}>
              ...
              <Link className="mt-4 inline-block font-bold text-amber-600" href={`/admin/quizzes/${quiz.id}/edit`}>Kelola soal</Link>
            </Card>
          ))}
        </div>
      ) : (
